...
pub async fn run_server(logger: Logger, config: Arc<RtmpServerConfiguration>) {
    ...
    tcp_server(Arc::new(logger.make_child_logger("[SERVER:TCP] ")), config.clone(), server_status.clone(), ip_counter.clone(), session_id_generator.clone(), end_notifier_tcp);

    if config.tls.is_enabled() {
        let (end_notifier_tls, mut end_receiver_tls) = tokio::sync::mpsc::channel::<()>(1);

        tls_server(Arc::new(logger.make_child_logger("[SERVER:TLS] ")), config.clone(), server_status.clone(), ip_counter.clone(), session_id_generator.clone(), end_notifier_tls);
