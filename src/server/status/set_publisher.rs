...
use crate::{server::{RtmpChannelStatus, RtmpServerContext}, session::{RtmpSessionMessage, RtmpSessionPublishStreamStatus, SessionReadThreadContext}, utils::string_compare_constant_time};
...
pub async fn set_publisher(
    ...
    match status.channels.get(channel) {
                    ...
                    if string_compare_constant_time(&player.provided_key, key) {
                        ...
                        let play_start_message =
                            RtmpSessionPublishStreamStatus::get_play_start_message(
                                &session_context.publish_status,
                                player.gop_clear,
                            )
                            .await;
        ...
        None => {
            ...
            new_channel_status.publisher_message_sender = Some(session_context.session_msg_sender.clone());
